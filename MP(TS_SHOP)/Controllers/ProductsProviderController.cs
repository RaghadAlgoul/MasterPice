...
namespace MP_TS_SHOP_.Controllers
{
    ...
    {
        private TS_SHOPEntitiess db = new TS_SHOPEntitiess();
        [Authorize]
        // GET: ProductsProvider
        public ActionResult Index()
        {
            string ASPid = User.Identity.GetUserId();
            var userid = db.Users.FirstOrDefault(m => m.Id == ASPid).UserId;
            ViewBag.Image= db.Users.FirstOrDefault(m => m.Id == ASPid).UserImage;
            ViewBag.name = db.Users.FirstOrDefault(m => m.Id == ASPid).Name;
            ...
            foreach (var item in db.Subscriptions)
            {
                if (item.ProviderId == userid)
                {
                    ...
                    var subId = item.SubscriptionNumber;
                    ...
                    //{
                        ...
                        if (subId == 1)
                        {
                            ...
                            if (DateTime.Now.Date.AddDays(2) == end1.Date)
                            {
            ...
            var products = db.Products.Where(x=>x.UserID== userid && x.State==true);
        ...
        public ActionResult Create([Bind(Include = "ProductId,CategoryId,State,Price,Size,Color,Description,Name,Image1,Image2,Image3,Image4,Image5,Image6,Image7,Image8,Image9,Image10,UserID")] Product product, HttpPostedFileBase Image1, HttpPostedFileBase Image2, HttpPostedFileBase Image3, HttpPostedFileBase Image4, HttpPostedFileBase Image5, HttpPostedFileBase Image6, HttpPostedFileBase Image7, HttpPostedFileBase Image8, HttpPostedFileBase Image9, HttpPostedFileBase Image10)
        {
            if (ModelState.IsValid)
            {
                ...
                product.UserID = id;
                product.ProductPostDate = DateTime.Now.Date;
        ...
        public ActionResult EditProfile()
        {
            ...
            return View(db.Users.Where(x=>x.Id==ASPid).ToList());
        }
        ...
        public ActionResult EditProfileAction(string Name, string phone ,  string location ,string faceBook ,string UserInstagram, string tweeter ,HttpPostedFileBase pic)
        {
            var ASPid = User.Identity.GetUserId();
            ...
            foreach (var item in db.Users) 
            {
                if(item.Id == ASPid)
                {
                    ...
                    if (faceBook != "" || faceBook != string.Empty)
                    {
                        item.UserFaceBook = faceBook;
                    }
                    if (UserInstagram != "" || UserInstagram != string.Empty)
                    {
                        item.UserInstagram = UserInstagram;
                    }
                    if (tweeter != "" || tweeter != string.Empty)
                    {
                        item.UserTweeter = tweeter;
                    }
                    if (pic != null && pic.ContentLength > 0)
                    {
                        string path = "../Image/" + Path.GetFileName(pic.FileName);
                        pic.SaveAs(Server.MapPath(path));
                        item.UserImage = Path.GetFileName(pic.FileName);
                    }
                    break;
                }
            ...
            db.SaveChanges();
            return RedirectToAction("Profile","ProductsProvider");
        }
